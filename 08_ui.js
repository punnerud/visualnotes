...
function setupBandDrag(el) {
  ...
  let justDragged = false, lastIdx = -1, snapTimer = null;
  const beatAt = x => clamp(beatAtBandOffset(x), 0, beatTotal);
  const follow = beat => {
    setLanesFromBeat(beat, 'none');
    if (curDir() !== 'v') {                    // kortstripen hopper til nærmeste tone
      const i = clamp(Math.round(indexAtBeat(beat)), 0, state.events.length - 1);
      if (i !== lastIdx) { lastIdx = i; centerCard(cards[i], true); }
    }
  };
  const snap = () => {
    if (!state.events.length) return;
    go(clamp(Math.round(indexAtBeat(beatAt(sX))), 0, state.events.length - 1));
  };
